...
def ori_demo():
    QuadPixcellSample = JPackage('net').epsilony.simpmeshfree.adpt2d.sample.QuadPixcellSample
    fig = plt.figure()
    fig.show()
    for smallbit in xrange(16):
        ax = fig.add_subplot(4, 4, smallbit + 1, aspect='equal')
        samples = QuadPixcellSample.refineSamples(0.0, 0.0, 5.0, smallbit)
...
def refine_demo():
    ...
    for smallbit in xrange(16):
        ...
        samples2 = QuadPixcellSample.refineSamples(0.0, 0.0, 5.0, smallbit)
